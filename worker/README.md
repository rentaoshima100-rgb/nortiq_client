...
before / after の切り出しを odiff で比べ、変化したピクセルを **#ff00ff** で塗った
画像を作って `request_shots.diff_path` / `diff_ratio` に記録する。

**判定には使わない。** 判定はレイアウトマップの結合（`compare.mjs`）で行う。
これは「クライアントに見せる視覚表現」専用。

切り出しは文言が伸びるなどで寸法が変わる。odiff は同寸でないと比較できないので、
大きい方に**白で余白を足して**揃えてから比べる。

実測: 見出しを1行書き換えたケースで **7.13% のピクセルが変化**。
変わった文字だけが正確に着色された。
