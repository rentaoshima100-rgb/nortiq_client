...
| **2** | **実装済み** | ラウンド、カウント、フリーズ（手動/無操作/件数上限）、確認・差し戻し・自動確認、無償枠を超えたラウンドの有償化 |
| **1** | **一部** | 注入の可否を確定し `tools/nq-inject` を実装。撮影・レイアウトマップ・差分・リプレイ検証を `worker/` に実装（**完了条件②を達成**）。**残: Storage/DB 保存、切り出し、GitHub/PR 基盤** |
